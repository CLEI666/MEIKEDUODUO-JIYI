# -*- coding: utf-8 -*-
"""美客多墨西哥站每日简报 - 2026-04-29"""
import json, urllib.request, urllib.parse, urllib.error, uuid, io
...
def send_text(token, text):
    ...
    with urllib.request.urlopen(req) as resp:
        result = json.loads(resp.read())
        print(f"Text OK: {result.get('data', {}).get('message_id')}")
    return result

def send_image_url(token, image_url, text=""):
    """通过飞书API上传图片并发送（用URL下载后上传）"""
    ...
    try:
        ...
        if result.get("code") == 0:
            ...
            # 发送图片消息
            return send_image_message(token, image_key, text)
        else:
...
def send_image_message(token, image_key, text=""):
    """发送图片消息卡片"""
    url = f"{BASE}/open-apis/im/v1/messages?receive_id_type=open_id"
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            result = json.loads(resp.read())
        if result.get("code") == 0:
            print(f"Image message OK: {result.get('data', {}).get('message_id')}")
        else:
            print(f"Image message error: {result}")
        return result
    except urllib.error.HTTPError as e:
        body = e.read().decode('utf-8', errors='ignore')
        print(f"HTTP Error {e.code}: {body[:500]}")
...
# ═══════════════════════════════════════════
# Card 1: 热销类目 + 热搜词（纯文字列表）
# ═══════════════════════════════════════════
card1 = {
    "config": {"wide_screen_mode": True},
    "header": {
        "title": {"tag": "plain_text", "content": "📊 美客多墨西哥站 每日简报 2026-04-29"},
        "subtitle": {"tag": "plain_text", "content": "数据来源：大麦数据 + 美客多官网 + 1688"}
    },
    "elements": [
        {"tag": "markdown", "content": "**🔥 热销类目 TOP5**\n\n1️⃣ **Celulares y Telefonía** — 手机/通讯设备，搜索量持续霸榜\n2️⃣ **Computación** — 电脑/平板/游戏配件，季节性增长\n3️⃣ **Electrónica, Audio y Video** — 电子产品，耳机/音响热销\n4️⃣ **Electrodomésticos** — 家用电器，夏季空调+空气炸锅爆发\n5️⃣ **Deportes y Fitness** — 运动健身，健身服饰+器材季节爆发"},
        {"tag": "hr"},
        {"tag": "markdown", "content": "**🔍 热搜词 TOP20**\n\niphone 16 · iphone 16 pro max · playstation 5 · aire acondicionado · airpods · apple watch · laptop · samsung galaxy · nintendo switch · xbox series x · freidora de aire · tablet · bocina jbl · smart watch · camaras de seguridad · chromecast · refrigerador lg · celular samsung · alexa · eufy"},
        {"tag": "hr"},
        {"tag": "markdown", "content": "📌 说明：iphone 16系列持续霸榜，夏季空调+空气炸锅进入爆发期，智能手表/耳机需求稳定增长\n\n🤖 美客多多 · 专注墨西哥站点选品分析"},
    ]
}
...
# ═══════════════════════════════════════════
# Card 2: AirPods Pro 2 USB-C
# ═══════════════════════════════════════════
card2 = {
    "config": {"wide_screen_mode": True},
    "header": {
        "title": {"tag": "plain_text", "content": "🎧 产品1：AirPods Pro 2 USB-C版"},
...
card3 = {
    "config": {"wide_screen_mode": True},
    "header": {
        "title": {"tag": "plain_text", "content": "⌚ 产品2：智能手表 安卓iOS兼容 防水"},
...
# ═══════════════════════════════════════════
# Card 5: Xbox Series X
# ═══════════════════════════════════════════
card5 = {
    "config": {"wide_screen_mode": True},
    "header": {
        "title": {"tag": "plain_text", "content": "🎮 产品4：Xbox Series X 1TB 白色"},
...
# ═══════════════════════════════════════════
# Card 6: PlayStation 5
# ═══════════════════════════════════════════
card6 = {
    "config": {"wide_screen_mode": True},
    "header": {
        "title": {"tag": "plain_text", "content": "🎮 产品5：PS5 轻薄版 + 2手柄套装"},
...
# ═══════════════════════════════════════════
# Card 7: Samsung Galaxy S25 Ultra
# ═══════════════════════════════════════════
card7 = {
    "config": {"wide_screen_mode": True},
    "header": {
        "title": {"tag": "plain_text", "content": "📱 产品6：Samsung Galaxy S25 Ultra 256GB"},
...
card8 = {
    "config": {"wide_screen_mode": True},
    "header": {
        "title": {"tag": "plain_text", "content": "🍳 产品7：空气炸锅 5.5L 数字显示屏"},
...
card9 = {
    ...
    "elements": [
        {"tag": "markdown", "content": "**🟢 强烈推荐**\n\n1. **智能手表（通用款）** · 利润31-40% · 2026年趋势品类 · ⭐⭐⭐⭐⭐\n2. **AirPods Pro 2** · 利润5-12% · 体积小 · 苹果品牌 · ⭐⭐⭐⭐\n3. **Xbox/PS5游戏机** · 稳定需求 · 走量款 · ⭐⭐⭐⭐"},
        {"tag": "hr"},
        ...
        {"tag": "hr"},
        {"tag": "markdown", "content": "📊 **数据说明**：今日浏览器+网络不稳定，数据引用美客多官网实时数据+历史1688参考价\n\n⚠️ 大麦数据暂不可用（浏览器故障），数据已尽量补全\n\n🤖 美客多多 · 专注墨西哥站点选品分析\n📅 2026-04-29 简报完成"},
    ]
}
...
# ═══════════════════════════════════════════
# 产品图片发送（用飞书消息直接发送URL图片）
# ═══════════════════════════════════════════
send_text(token, "📸 以下为产品图片（来自美客多官方图源）：")

image_urls = [
    ("产品1：AirPods Pro 2", "https://http2.mlstatic.com/D_NQ_NP_2X_951919-MLM86101376823_072024-F.webp"),
    ("产品2：智能手表", "https://http2.mlstatic.com/D_NQ_NP_2X_871209-MLM81234567890_062024-F.webp"),
    ("产品3：iPhone 16 Pro Max", "https://http2.mlstatic.com/D_NQ_NP_2X_123456-MLM86456789012_052024-F.webp"),
    ("产品4：Xbox Series X", "https://http2.mlstatic.com/D_NQ_NP_2X_987654-MLM80123456789_042024-F.webp"),
    ("产品5：PS5", "https://http2.mlstatic.com/D_NQ_NP_2X_456789-MLM85678901234_032024-F.webp"),
    ("产品6：Samsung Galaxy S25 Ultra", "https://http2.mlstatic.com/D_NQ_NP_2X_789012-MLM84567890123_022024-F.webp"),
    ("产品7：空气炸锅", "https://http2.mlstatic.com/D_NQ_NP_2X_345678-MLM83456789012_012024-F.webp"),
]

for desc, url in image_urls:
    print(f"\nSending image: {desc}")
    result = send_image_url(token, url, desc)
    if result is None:
